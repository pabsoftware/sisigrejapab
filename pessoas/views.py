...
def pesquisar_pessoas(request):
    ...
    if pesquisa_nome:
        object_list = Pessoas.objects.all().filter(nome__contains=pesquisa_nome).order_by('nome')   
    ...
    if zona_id and area_id:
       object_list = Pessoas.objects.all().filter(zona_id=zona_id, area_id=area_id).order_by('nome')
    
     
    if zona_id and  area_id and congregacao_id:
       object_list = Pessoas.objects.all().filter( zona_id=zona_id, area_id=area_id, congregacao_id=congregacao_id).order_by('nome')
    
  
    if cargo_id and zona_id:
       object_list = Pessoas.objects.all().filter(zona_id=zona_id, cargo_id=cargo_id).order_by('nome')

    if cargo_id and zona_id and area_id:
       object_list = Pessoas.objects.all().filter(zona_id=zona_id, area_id=area_id, cargo_id=cargo_id).order_by('nome')

    if zona_id and  area_id and congregacao_id and cargo_id:
       object_list = Pessoas.objects.all().filter( zona_id=zona_id, area_id=area_id, congregacao_id=congregacao_id, cargo_id=cargo_id).order_by('nome')
    
    if zona_id and  area_id and congregacao_id and cargo_id and pesquisa_nome:
       object_list = Pessoas.objects.all().filter( zona_id=zona_id, area_id=area_id, congregacao_id=congregacao_id, cargo_id=cargo_id, nome__contains=pesquisa_nome).order_by('nome')
